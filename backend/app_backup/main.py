...
async def _ensure_runtime_schema():
    """기존 DB와의 하위 호환: 세부 카테고리 컬럼이 없으면 추가."""
    async with engine.begin() as conn:
        await conn.execute(
            text("ALTER TABLE transactions ADD COLUMN IF NOT EXISTS subcategory TEXT NOT NULL DEFAULT '기타'")
        )
        await conn.execute(
            text(
                "CREATE INDEX IF NOT EXISTS idx_transactions_user_subcategory "
                "ON transactions (user_id, subcategory)"
            )
        )
