...
from app.services.normalizer import normalize_amount, normalize_date, resolve_category_subcategory
...
async def create_transaction(
    ...
    date/amount/category/subcategory는 서버에서 정규화 적용.
    """
    # idempotency: 캐시 확인
    if body.idem_key:
    ...
    result = await session.execute(
        text("""
            INSERT INTO transactions (
                user_id, occurred_date, type, amount, currency, category, subcategory, merchant, memo, source_text
            )
            VALUES (
                :user_id, :occurred_date, :type, :amount, :currency, :category, :subcategory, :merchant, :memo, :source_text
            )
...
async def list_transactions(
    ...
    result = await session.execute(
        text(f"""
            SELECT tx_id, user_id, occurred_date, type, amount, currency, category, subcategory, merchant, memo, source_text, created_at
            FROM transactions
            WHERE {where_clause}
